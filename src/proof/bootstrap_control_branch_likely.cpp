...
void run_branch_likely_demo(
  ...
  if (expect_taken) {
    ...
    if (machine.inspect_cpu_gpr(kDelaySlotMarkerIndex) != 0 ||
        machine.inspect_cpu_gpr(kFallthroughMarkerIndex) != 0 ||
    ...
    if (machine.inspect_cpu_gpr(kFallthroughMarkerIndex) != 0) {
      throw std::runtime_error(
  ...
  if (machine.cpu_pc() != cpu_rdram_alias(kFallthroughAddress)) {
    throw std::runtime_error(
        std::string("branch-likely not-taken demo did not skip the delay slot: ") + label);
  }

  if (machine.cpu_next_pc() != cpu_rdram_alias(kNotTakenSentinelAddress)) {
    throw std::runtime_error(
  ...
  if (machine.inspect_cpu_gpr(kFallthroughMarkerIndex) !=
...
void run_branch_likely_link_demo(
    ...
    const char* label,
	    ...
	    CpuRegisterValue rs_value,
	    CpuRegisterValue initial_link_value,
	    CpuRegisterValue expected_not_taken_link_value,
	    bool expect_taken,
	    std::uint16_t delay_slot_marker,
	    std::uint16_t fallthrough_marker,
    std::uint16_t target_marker) {
  ...
  const std::uint32_t kTargetAddress = base_address + 16u;
  const std::uint32_t kTakenSentinelAddress = base_address + 20u;
  const std::uint32_t kExpectedLinkValue = cpu_rdram_alias(base_address + 8u);
  ...
  machine.stage_cpu_pc(cpu_rdram_alias(kBranchAddress));
  machine.stage_cpu_gpr(kLinkIndex, initial_link_value);
  machine.stage_cpu_gpr(rs_index, rs_value);
  machine.stage_cpu_gpr(kDelaySlotMarkerIndex, 0);
  machine.stage_cpu_gpr(kFallthroughMarkerIndex, 0);
  machine.stage_cpu_gpr(kTargetMarkerIndex, 0);
  ...
  std::cout << "before branch step:\n";
  print_control_flow_state(machine);
  print_hex64("  rs", machine.inspect_cpu_gpr(rs_index));
  print_hex64("  gpr[6]", machine.inspect_cpu_gpr(kDelaySlotMarkerIndex));
  print_hex64("  gpr[7]", machine.inspect_cpu_gpr(kFallthroughMarkerIndex));
  print_hex64("  gpr[8]", machine.inspect_cpu_gpr(kTargetMarkerIndex));
  ...
  if (machine.inspect_cpu_gpr(kLinkIndex) != expected_not_taken_link_value) {
    throw std::runtime_error(
        std::string("branch-likely link not-taken demo wrote the wrong link register value: ") + label);
  }
