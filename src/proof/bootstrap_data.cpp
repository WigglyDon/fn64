...
constexpr std::uint32_t kPiStatusClearInterrupt = 0x00000002u;
constexpr std::uint32_t kPiStatusUnsupportedBits = 0x00000004u;
...
constexpr std::uint32_t kSpStatusClearInterrupt = 0x00000008u;
constexpr std::uint32_t kSpStatusUnsupportedBits = 0x00000010u;
constexpr std::uint32_t kSpStatusSetInterruptUnsupported = 0x00000040u;
...
void write_pi_register_through_cpu(
    Machine& machine,
    RdramOffset instruction_address,
    std::size_t source_register,
...
void require_pi_register_equals(
    Machine& machine,
    RdramOffset instruction_address,
...
void write_sp_register_through_cpu(
    Machine& machine,
    RdramOffset instruction_address,
    std::size_t source_register,
    std::size_t base_register,
...
void require_sp_register_equals(
    Machine& machine,
    RdramOffset instruction_address,
    std::size_t target_register,
    std::size_t base_register,
...
void write_si_register_through_cpu(
    Machine& machine,
    RdramOffset instruction_address,
    std::size_t source_register,
    std::size_t base_register,
...
void require_cartridge_ipl3_candidate_entry_boundaries(
    const std::vector<std::uint8_t>& normalized_bytes) {
  auto machine_storage = make_ipl3_staging_proof_machine(
      normalized_bytes,
      RomSourceLayout::kBigEndian,
      "cartridge_ipl3_entry_boundaries");
  Machine& machine = *machine_storage;

  constexpr std::size_t kBaseIndex = 4;
  constexpr std::size_t kSourceIndex = 5;
  constexpr std::size_t kTargetIndex = 6;
  constexpr RdramOffset kSeedDmemStoreAddress = 0x00004a20u;
  constexpr RdramOffset kSeedImemStoreAddress = 0x00004a24u;
  constexpr RdramOffset kCountReadAddress = 0x00004a28u;
  constexpr RdramOffset kDmemEntryReadAddress = 0x00004a2cu;
  constexpr RdramOffset kImemReadAddress = 0x00004a30u;
  constexpr RdramOffset kRdramSentinelAddress = 0x00004c20u;
  constexpr std::uint32_t kDmemEntrySentinel = 0x11223344u;
  constexpr std::uint32_t kImemSentinel = 0x55667788u;
  constexpr std::uint32_t kRdramSentinel = 0xaabbccddu;
  constexpr CpuAddress kInitialPc = 0x80004e20u;
  constexpr CpuAddress kInitialNextPc = 0x80004e24u;
  const std::uint32_t cartridge_header_entry =
      read_synthetic_be_u32(normalized_bytes, kCartridgeHeaderEntryWordOffset);
  const std::uint32_t cartridge_ipl3_first_word =
      read_synthetic_be_u32(normalized_bytes, kCartridgeCandidateIpl3StartOffset);
  const std::uint8_t cartridge_first_byte =
      machine.cartridge().read_u8(kCartridgeCandidateIpl3StartOffset);
  ...
  machine.stage_cpu_gpr(kBaseIndex, sp_dmem_uncached_alias(kCartridgeCandidateIpl3StartOffset));
  machine.stage_cpu_gpr(kSourceIndex, kDmemEntrySentinel);
  step_at(machine, kSeedDmemStoreAddress, "cartridge_ipl3_entry_seed_dmem");
  machine.stage_cpu_gpr(kBaseIndex, sp_imem_uncached_alias(kCartridgeCandidateIpl3StartOffset));
  machine.stage_cpu_gpr(kSourceIndex, kImemSentinel);
  step_at(machine, kSeedImemStoreAddress, "cartridge_ipl3_entry_seed_imem");

  machine.stage_rdram_u32_be(kRdramSentinelAddress, kRdramSentinel);
  machine.stage_cpu_pc(kInitialPc);
  machine.stage_cpu_next_pc(kInitialNextPc);
  machine.enter_sp_dmem_ipl3_candidate();

  if (machine.cpu_pc() != Machine::kSpDmemIpl3CandidateEntryPc ||
      machine.cpu_next_pc() != Machine::kSpDmemIpl3CandidateEntryNextPc) {
    throw std::runtime_error("cartridge_ipl3_entry did not select the named entry");
  }
  if (machine.cpu_pc() == kInitialPc ||
      machine.cpu_next_pc() == kInitialNextPc) {
    throw std::runtime_error("cartridge_ipl3_entry left the old pc/next_pc");
  }
  if (machine.cpu_pc() == cartridge_header_entry) {
    throw std::runtime_error("cartridge_ipl3_entry used cartridge header entry");
  }
  require_cop0_register_equals(
      machine,
      kCountReadAddress,
      ...
      kCop0CountRegisterIndex,
      2,
      "cartridge_ipl3_entry_count_unchanged");
  require_rdram_word_equals(
      machine,
      kRdramSentinelAddress,
      kRdramSentinel,
      "cartridge_ipl3_entry_rdram_unchanged");
  require_sp_word_through_cpu_data_path(
      machine,
      kDmemEntryReadAddress,
      kBaseIndex,
      kTargetIndex,
      sp_dmem_uncached_alias(kCartridgeCandidateIpl3StartOffset),
      kDmemEntrySentinel,
      "cartridge_ipl3_entry_dmem_unchanged");
  if (kDmemEntrySentinel == cartridge_ipl3_first_word) {
    throw std::runtime_error("cartridge_ipl3_entry sentinel accidentally matches cartridge word");
  }
  require_sp_word_through_cpu_data_path(
      machine,
      kImemReadAddress,
      ...
      kImemSentinel,
      "cartridge_ipl3_entry_imem_unchanged");
  if (machine.cartridge().read_u8(kCartridgeCandidateIpl3StartOffset) !=
      cartridge_first_byte) {
    throw std::runtime_error("cartridge_ipl3_entry mutated cartridge bytes");
  }
}
...
void require_cartridge_ipl3_staging_boundaries(
  ...
  if (machine.cpu_pc() == Machine::kSpDmemIpl3CandidateEntryPc ||
      machine.cpu_next_pc() == Machine::kSpDmemIpl3CandidateEntryNextPc) {
    throw std::runtime_error("cartridge_ipl3_stage entered IPL3 candidate without entry call");
  }
...
void require_cartridge_ipl3_staged_fetch_after_named_entry(
    const std::vector<std::uint8_t>& normalized_bytes) {
  auto machine_storage = make_ipl3_staging_proof_machine(
      normalized_bytes,
      RomSourceLayout::kBigEndian,
  ...
  require_gpr_equals(
      ...
      "cartridge_ipl3_stage_explicit_fetch_lui");
  if (machine.cpu_pc() != Machine::kSpDmemIpl3CandidateEntryPc + 4u ||
      machine.cpu_next_pc() != Machine::kSpDmemIpl3CandidateEntryPc + 8u) {
...
void run_cartridge_ipl3_candidate_staging_demo() {
  ...
  require_cartridge_ipl3_candidate_entry_boundaries(normalized_bytes);
  require_cartridge_ipl3_staging_boundaries(normalized_bytes);
  require_cartridge_ipl3_staged_fetch_after_named_entry(normalized_bytes);
...
void run_pi_mmio_fault_demo() {
  ...
  const auto require_pi_machine_fault =
      [](const char* label,
        ...
        constexpr std::size_t kSourceIndex = 5;
        constexpr std::size_t kTargetIndex = 6;
        constexpr RdramOffset kStatusWriteAddress = 0x000011e0u;
...
void run_si_pif_dma_success_demo() {
  ...
  require_si_register_equals(
      ...
      "si_success_demo_zero_dma_status_pending");
  require_mi_register_equals(
      machine,
      kLwMiPendingAddress,
      kReadIndex,
  ...
  require_mi_register_equals(
      ...
      kMiInterruptPendingRegisterOffset,
      0,
      "si_success_demo_mi_clear_cleared_mi_only");
  require_si_register_equals(
      machine,
      kLwStatusAddress,
      kReadIndex,
      ...
      kSiStatusRegisterOffset,
      kSiStatusInterruptPending,
      "si_success_demo_mi_clear_left_si_status");
  write_si_register_through_cpu(
      ...
      kSiStatusRegisterOffset,
      kSiStatusInterruptPending,
      "si_success_demo_ack_zero_dma_status");
  require_si_register_equals(
      machine,
      kLwStatusAddress,
      kReadIndex,
      ...
      kSiStatusRegisterOffset,
      0,
      "si_success_demo_ack_cleared_zero_dma_status");
  ...
  require_si_register_equals(
      ...
      kSiStatusRegisterOffset,
      kSiStatusInterruptPending,
      "si_success_demo_dram_to_pif_status_pending");
  write_si_register_through_cpu(
      machine,
      kSwStatusAddress,
      kValueIndex,
      ...
      kSiStatusRegisterOffset,
      0,
      "si_success_demo_status_zero_keeps_pending");
  require_si_register_equals(
      machine,
      kLwStatusAddress,
      kReadIndex,
      ...
      kSiStatusRegisterOffset,
      kSiStatusInterruptPending,
      "si_success_demo_status_zero_left_status");
  require_mi_register_equals(
      machine,
      kLwMiPendingAddress,
      kReadIndex,
      ...
      kMiInterruptPendingRegisterOffset,
      kMiPendingSi,
      "si_success_demo_status_zero_left_mi_pending");
  write_si_register_through_cpu(
      machine,
      kSwStatusAddress,
      kValueIndex,
      kSiBaseIndex,
      kSiStatusRegisterOffset,
      kSiStatusUnsupportedBits,
      "si_success_demo_status_unsupported_keeps_pending");
  require_si_register_equals(
      machine,
      kLwStatusAddress,
      kReadIndex,
      ...
      kSiStatusRegisterOffset,
      kSiStatusInterruptPending,
      "si_success_demo_status_unsupported_left_status");
  require_mi_register_equals(
      machine,
      kLwMiPendingAddress,
      kReadIndex,
      ...
      kMiInterruptPendingRegisterOffset,
      kMiPendingSi,
      "si_success_demo_status_unsupported_left_mi_pending");
  ...
  require_si_register_equals(
      ...
      kSiStatusRegisterOffset,
      kSiStatusInterruptPending,
      "si_success_demo_pif_to_dram_status_pending");
}

void run_si_status_ack_demo() {
  std::cout << "fn64 bootstrap SI MMIO demo: local SI status acknowledgement\n";

  {
    ...
    constexpr RdramOffset kSwPiDramAddress = 0x000016b0u;
    constexpr RdramOffset kSwPiCartAddress = 0x000016b4u;
    constexpr RdramOffset kSwPiLengthAddress = 0x000016b8u;
    constexpr RdramOffset kSwSiDramAddress = 0x000016bcu;
    constexpr RdramOffset kSwSiDramToPifAddress = 0x000016c0u;
    constexpr RdramOffset kSwSiPifToDramAddress = 0x000016c4u;
    constexpr RdramOffset kSwSiStatusAddress = 0x000016c8u;
    constexpr RdramOffset kLwSiStatusAddress = 0x000016ccu;
    constexpr RdramOffset kLwSiDramAddress = 0x000016d0u;
    constexpr RdramOffset kLwSiDramToPifAddress = 0x000016d4u;
    constexpr RdramOffset kLwMiPendingAddress = 0x000016d8u;
    constexpr RdramOffset kSwMiMaskAddress = 0x000016dcu;
    constexpr RdramOffset kLwMiMaskAddress = 0x000016e0u;
    constexpr RdramOffset kCauseReadAddress = 0x000016e4u;
    constexpr RdramOffset kPiDestinationAddress = 0x00003c00u;
    constexpr RdramOffset kSiSourceAddress = 0x00003d00u;
    constexpr RdramOffset kSiRoundTripAddress = 0x00003d80u;
    ...
    stage_si_sw_instruction(
        machine,
        kSwSiPifToDramAddress,
        kValueIndex,
        kSiBaseIndex,
        kSiPifToDramRegisterOffset);
    stage_si_sw_instruction(machine, kSwSiStatusAddress, kValueIndex, kSiBaseIndex, kSiStatusRegisterOffset);

    machine.stage_cpu_gpr(kPiBaseIndex, kSyntheticPiMmioCpuBase);
    machine.stage_cpu_gpr(kSiBaseIndex, kSyntheticSiMmioCpuBase);
    machine.stage_cpu_gpr(kMiBaseIndex, kSyntheticMiMmioCpuBase);

    machine.stage_cpu_gpr(kValueIndex, kPiDestinationAddress);
    step_at(machine, kSwPiDramAddress, "si_status_ack_demo_pi_sw_dram");
    machine.stage_cpu_gpr(kValueIndex, pi_cart_rom_address(0x00000040u));
    step_at(machine, kSwPiCartAddress, "si_status_ack_demo_pi_sw_cart");
    machine.stage_cpu_gpr(kValueIndex, 3u);
    step_at(machine, kSwPiLengthAddress, "si_status_ack_demo_pi_dma");

    stage_si_dma_pattern(machine, kSiSourceAddress, 0xa1u);
    stage_si_dma_zeroes(machine, kSiRoundTripAddress);
    machine.stage_cpu_gpr(kValueIndex, kSiSourceAddress);
    step_at(machine, kSwSiDramAddress, "si_status_ack_demo_si_sw_dram");
    machine.stage_cpu_gpr(kValueIndex, kSiSupportedPifRamAddress);
    step_at(machine, kSwSiDramToPifAddress, "si_status_ack_demo_si_dma");
    require_si_register_equals(
        machine,
        kLwSiStatusAddress,
        kReadIndex,
        ...
        kSiStatusRegisterOffset,
        kSiStatusInterruptPending,
        "si_status_ack_demo_status_pending");
    require_mi_register_equals(
        machine,
        kLwMiPendingAddress,
        kReadIndex,
        ...
        "si_status_ack_demo_mi_pending_pi_si");
    ...
    write_mi_register_through_cpu(
        ...
        kMiInterruptMaskRegisterOffset,
        kMiPendingSi,
        "si_status_ack_demo_write_si_mask");
    require_cop0_register_equals(
        machine,
        kCauseReadAddress,
        kCauseReadIndex,
        kCop0CauseRegisterIndex,
        kCop0CauseIp2,
        "si_status_ack_demo_cause_ip2_before_ack");
    write_si_register_through_cpu(
        machine,
        kSwSiStatusAddress,
        kValueIndex,
    ...
    require_si_register_equals(
        ...
        kSiStatusRegisterOffset,
        0,
        "si_status_ack_demo_status_cleared");
    require_mi_register_equals(
        machine,
        kLwMiPendingAddress,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingPi,
        "si_status_ack_demo_si_clear_preserves_pi");
    require_mi_register_equals(
        machine,
        kLwMiMaskAddress,
        kReadIndex,
        ...
        kMiInterruptMaskRegisterOffset,
        kMiPendingSi,
        "si_status_ack_demo_mask_unchanged");
    require_cop0_register_equals(
        machine,
        kCauseReadAddress,
        kCauseReadIndex,
        kCop0CauseRegisterIndex,
        0,
        "si_status_ack_demo_cause_ip2_after_ack");
    require_si_register_equals(
        machine,
        kLwSiDramAddress,
        kReadIndex,
        ...
        kSiDramRegisterOffset,
        kSiSourceAddress,
        "si_status_ack_demo_dram_register_unchanged");
    require_si_register_equals(
        machine,
        kLwSiDramToPifAddress,
        kReadIndex,
        kSiBaseIndex,
        kSiDramToPifRegisterOffset,
        kSiSupportedPifRamAddress,
        "si_status_ack_demo_trigger_shadow_unchanged");

    machine.stage_cpu_gpr(kValueIndex, kSiRoundTripAddress);
    step_at(machine, kSwSiDramAddress, "si_status_ack_demo_roundtrip_sw_dram");
    machine.stage_cpu_gpr(kValueIndex, kSiSupportedPifRamAddress);
    step_at(machine, kSwSiPifToDramAddress, "si_status_ack_demo_roundtrip_dma");
    require_si_dma_pattern(machine, kSiRoundTripAddress, 0xa1u, "si_status_ack_demo_pif_unchanged");
  }

  {
    ...
    constexpr std::size_t kValueIndex = 6;
    constexpr std::size_t kReadIndex = 7;
    constexpr RdramOffset kSwSiDramAddress = 0x000016b0u;
    constexpr RdramOffset kSwSiDramToPifAddress = 0x000016b4u;
    constexpr RdramOffset kSwSiStatusAddress = 0x000016b8u;
    constexpr RdramOffset kLwSiStatusAddress = 0x000016bcu;
    constexpr RdramOffset kLwMiPendingAddress = 0x000016c0u;
    constexpr RdramOffset kSiSourceAddress = 0x00003e00u;
    ...
    machine.stage_cpu_gpr(kSiBaseIndex, kSyntheticSiMmioCpuBase);
    machine.stage_cpu_gpr(kMiBaseIndex, kSyntheticMiMmioCpuBase);
    stage_si_dma_pattern(machine, kSiSourceAddress, 0xb1u);

    machine.stage_cpu_gpr(kValueIndex, kSiSourceAddress);
    step_at(machine, kSwSiDramAddress, "si_status_noop_demo_sw_dram");
    machine.stage_cpu_gpr(kValueIndex, kSiSupportedPifRamAddress);
    step_at(machine, kSwSiDramToPifAddress, "si_status_noop_demo_dma");
    write_si_register_through_cpu(
        machine,
        kSwSiStatusAddress,
        kValueIndex,
    ...
    require_si_register_equals(
        ...
        kSiStatusRegisterOffset,
        kSiStatusInterruptPending,
        "si_status_noop_demo_zero_left_status");
    require_mi_register_equals(
        machine,
        kLwMiPendingAddress,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingSi,
        "si_status_noop_demo_zero_left_mi");
    write_si_register_through_cpu(
        machine,
        kSwSiStatusAddress,
        kValueIndex,
        ...
        kSiStatusRegisterOffset,
        kSiStatusUnsupportedBits,
        "si_status_noop_demo_unsupported");
    require_si_register_equals(
        machine,
        kLwSiStatusAddress,
        kReadIndex,
        ...
        kSiStatusRegisterOffset,
        kSiStatusInterruptPending,
        "si_status_noop_demo_unsupported_left_status");
    require_mi_register_equals(
        machine,
        kLwMiPendingAddress,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingSi,
        "si_status_noop_demo_unsupported_left_mi");
  }

  {
    ...
    constexpr std::size_t kValueIndex = 6;
    constexpr std::size_t kReadIndex = 7;
    constexpr RdramOffset kSwSiDramAddress = 0x000016d0u;
    constexpr RdramOffset kSwSiDramToPifAddress = 0x000016d4u;
    constexpr RdramOffset kSwSiStatusAddress = 0x000016d8u;
    constexpr RdramOffset kSwMiPendingAddress = 0x000016dcu;
    constexpr RdramOffset kLwSiStatusAddress = 0x000016e0u;
    constexpr RdramOffset kLwMiPendingAddress = 0x000016e4u;
    constexpr RdramOffset kSiSourceAddress = 0x00003e80u;
    ...
    machine.stage_cpu_gpr(kSiBaseIndex, kSyntheticSiMmioCpuBase);
    machine.stage_cpu_gpr(kMiBaseIndex, kSyntheticMiMmioCpuBase);
    stage_si_dma_pattern(machine, kSiSourceAddress, 0xc1u);

    machine.stage_cpu_gpr(kValueIndex, kSiSourceAddress);
    step_at(machine, kSwSiDramAddress, "si_status_mi_clear_demo_sw_dram");
    machine.stage_cpu_gpr(kValueIndex, kSiSupportedPifRamAddress);
    step_at(machine, kSwSiDramToPifAddress, "si_status_mi_clear_demo_dma");
    write_mi_register_through_cpu(
        machine,
        kSwMiPendingAddress,
        kValueIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingSi,
        "si_status_mi_clear_demo_clear_mi");
    require_mi_register_equals(
        machine,
        kLwMiPendingAddress,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        0,
        "si_status_mi_clear_demo_mi_cleared");
    require_si_register_equals(
        machine,
        kLwSiStatusAddress,
        kReadIndex,
        ...
        kSiStatusRegisterOffset,
        kSiStatusInterruptPending,
        "si_status_mi_clear_demo_status_still_pending");
    write_si_register_through_cpu(
        machine,
        kSwSiStatusAddress,
        kValueIndex,
        ...
        kSiStatusRegisterOffset,
        kSiStatusInterruptPending,
        "si_status_mi_clear_demo_ack_status");
    require_si_register_equals(
        machine,
        kLwSiStatusAddress,
        kReadIndex,
        ...
        kSiStatusRegisterOffset,
        0,
        "si_status_mi_clear_demo_status_cleared");
  }

  {
    ...
    constexpr RdramOffset kSwSiDramToPifAddress = 0x000016b4u;
    constexpr RdramOffset kSwSiStatusAddress = 0x000016b8u;
    constexpr RdramOffset kSwMiMaskAddress = 0x000016bcu;
    constexpr RdramOffset kCauseWriteAddress = 0x000016c0u;
    constexpr RdramOffset kStatusWriteAddress = 0x000016c4u;
    constexpr RdramOffset kEpcWriteAddress = 0x000016c8u;
    constexpr RdramOffset kCountWriteAddress = 0x000016ccu;
    constexpr RdramOffset kCompareWriteAddress = 0x000016d0u;
    constexpr RdramOffset kCauseReadAddress = 0x000016d4u;
    constexpr RdramOffset kStatusReadAddress = 0x000016d8u;
    constexpr RdramOffset kEpcReadAddress = 0x000016dcu;
    constexpr RdramOffset kSiSourceAddress = 0x00003f00u;
    constexpr std::uint32_t kStatusValue =
        kCop0StatusInterruptMask0 |
        kCop0StatusInterruptMask2 |
        kCop0StatusInterruptMask7;
    constexpr CpuAddress kEpcValue = 0x80002000u;
    ...
    machine.stage_cpu_gpr(kSiBaseIndex, kSyntheticSiMmioCpuBase);
    machine.stage_cpu_gpr(kMiBaseIndex, kSyntheticMiMmioCpuBase);
    stage_si_dma_pattern(machine, kSiSourceAddress, 0xd1u);

    machine.stage_cpu_gpr(kValueIndex, kSiSourceAddress);
    step_at(machine, kSwSiDramAddress, "si_status_cop0_demo_sw_dram");
    machine.stage_cpu_gpr(kValueIndex, kSiSupportedPifRamAddress);
    step_at(machine, kSwSiDramToPifAddress, "si_status_cop0_demo_dma");
    write_mi_register_through_cpu(
        machine,
        kSwMiMaskAddress,
        kValueIndex,
        ...
        kMiInterruptMaskRegisterOffset,
        kMiPendingSi,
        "si_status_cop0_demo_write_mi_mask");
    write_cop0_register_through_cpu(
        machine,
        kStatusWriteAddress,
        kValueIndex,
        kCop0StatusRegisterIndex,
        kStatusValue,
        "si_status_cop0_demo_write_status");
    write_cop0_register_through_cpu(
        machine,
        kEpcWriteAddress,
        kValueIndex,
    ...
    write_cop0_register_through_cpu(
        ...
        kCop0CountRegisterIndex,
        0x00000100u,
        "si_status_cop0_demo_write_count");
    write_cop0_register_through_cpu(
        machine,
        ...
        0x00000102u,
        "si_status_cop0_demo_write_compare");
    write_cop0_register_through_cpu(
        machine,
        kCauseWriteAddress,
        kValueIndex,
        kCop0CauseRegisterIndex,
        kCop0CauseIp0,
        "si_status_cop0_demo_write_cause_ip0");
    require_cop0_register_equals(
        machine,
        kCauseReadAddress,
        kCauseReadIndex,
        ...
        "si_status_cop0_demo_cause_before_ack");
    write_si_register_through_cpu(
        machine,
        kSwSiStatusAddress,
        kValueIndex,
    ...
    require_cop0_register_equals(
        ...
        "si_status_cop0_demo_cause_after_ack");
    require_cop0_register_equals(
        machine,
        kStatusReadAddress,
        kStatusReadIndex,
        kCop0StatusRegisterIndex,
        kStatusValue,
        "si_status_cop0_demo_status_unchanged");
    require_cop0_register_equals(
        machine,
        kEpcReadAddress,
        kEpcReadIndex,
        kCop0EpcRegisterIndex,
        kEpcValue,
        "si_status_cop0_demo_epc_unchanged");
  }
}
...
void run_si_pif_dma_failure_demo() {
...

  {
    ...
    constexpr RdramOffset kSwStatusAddress = 0x000014dcu;
    constexpr RdramOffset kLwStatusAddress = 0x000014e0u;
    ...
    write_si_register_through_cpu(
        ...
        kSiStatusRegisterOffset,
        kSiStatusInterruptPending,
        "si_failure_demo_clear_seed_status");
    ...
    require_si_register_equals(
        ...
        kSiStatusRegisterOffset,
        0,
        "si_failure_demo_read_no_status");
    machine.stage_cpu_gpr(kScSourceIndex, 0xaabbccddu);
...

  {
    ...
    constexpr RdramOffset kSwStatusAddress = 0x0000151cu;
    constexpr RdramOffset kLwStatusAddress = 0x00001520u;
    ...
    write_si_register_through_cpu(
        ...
        kSiStatusRegisterOffset,
        kSiStatusInterruptPending,
        "si_failure_demo_write_clear_seed_status");
    ...
    require_si_register_equals(
        ...
        kSiStatusRegisterOffset,
        0,
        "si_failure_demo_write_no_status");
  ...
  const auto require_bad_pif_address_rejects =
      [](const char* label, bool pif_to_dram) {
        ...
        constexpr RdramOffset kSwStatusAddress = 0x00001554u;
        constexpr RdramOffset kLwStatusAddress = 0x00001558u;
...
void run_si_mi_interrupt_demo() {
...

  {
    ...
    stage_si_sw_instruction(machine, kVectorAckAddress, kValueIndex, kSiBaseIndex, kSiStatusRegisterOffset);
    ...
    machine.stage_cpu_gpr(kValueIndex, kSiStatusInterruptPending);
    ...
    require_stepped(machine.step_cpu_instruction(), "si_interrupt_demo_vector_ack");
    require_stepped(machine.step_cpu_instruction(), "si_interrupt_demo_eret");
    if (machine.cpu_pc() != cpu_rdram_alias(kOrdinaryAddress) ||
        machine.cpu_next_pc() != cpu_rdram_alias(kOrdinaryAddress + 4u)) {
      throw std::runtime_error("si_interrupt_demo ERET did not return to interrupted instruction");
    }
    require_stepped(machine.step_cpu_instruction(), "si_interrupt_demo_resumed_ordinary");
    require_gpr_equals(machine, kOrdinaryIndex, 0x5a5au, "si_interrupt_demo_resumed_ordinary");
  }
...
void run_sp_mmio_fault_demo() {
  ...
  const auto require_sp_mmio_machine_fault =
      [](const char* label,
        ...
        constexpr std::size_t kSourceIndex = 5;
        constexpr std::size_t kTargetIndex = 6;
        constexpr RdramOffset kStatusWriteAddress = 0x000017d0u;
...
void run_pi_sp_status_ack_demo() {
  std::cout << "fn64 bootstrap PI/SP MMIO demo: local status acknowledgement clears only owned MI pending bits\n";

  const auto trigger_pi_dma =
      [](Machine& machine,
         RdramOffset instruction_base,
         RdramOffset destination,
         std::size_t pi_base_register,
         std::size_t value_register,
         const char* label) {
        write_pi_register_through_cpu(
            machine,
            instruction_base,
            value_register,
            pi_base_register,
  ...
  const auto trigger_sp_read_dma =
      [](Machine& machine,
         RdramOffset instruction_base,
         RdramOffset source,
         std::uint32_t sp_offset,
         std::size_t sp_base_register,
         std::size_t value_register,
         const char* label) {
        machine.stage_rdram_u32_be(source, 0x11223344u);
        write_sp_register_through_cpu(
            machine,
            instruction_base,
            value_register,
...

  {
    ...
    constexpr std::size_t kMiBaseIndex = 6;
    constexpr std::size_t kReadIndex = 7;
    constexpr RdramOffset kReadBase = 0x00003f00u;

    machine.stage_cpu_gpr(kPiBaseIndex, kSyntheticPiMmioCpuBase);
    machine.stage_cpu_gpr(kSpBaseIndex, kSyntheticSpMmioCpuBase);
    machine.stage_cpu_gpr(kMiBaseIndex, kSyntheticMiMmioCpuBase);
    require_pi_register_equals(
        machine,
        kReadBase,
        kReadIndex,
        kPiBaseIndex,
        kPiStatusRegisterOffset,
        0,
        "pi_sp_status_ack_initial_pi_status");
    require_sp_register_equals(
        machine,
        kReadBase + 4u,
        kReadIndex,
        kSpBaseIndex,
        kSpStatusRegisterOffset,
        0,
        "pi_sp_status_ack_initial_sp_status");
    require_mi_register_equals(
        machine,
        kReadBase + 8u,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        0,
        "pi_sp_status_ack_initial_mi_pending");
    require_mi_register_equals(
        machine,
        kReadBase + 12u,
        kReadIndex,
        ...
        kMiInterruptMaskRegisterOffset,
        0,
        "pi_sp_status_ack_initial_mi_mask");
  }

  {
    ...
    constexpr RdramOffset kPiDmaInstructionBase = 0x00003f20u;
    constexpr RdramOffset kSwPiStatusAddress = 0x00003f2cu;
    constexpr RdramOffset kReadBase = 0x00003f30u;
    constexpr RdramOffset kSwMiMaskAddress = 0x00003f50u;
    constexpr RdramOffset kCauseReadAddress = 0x00003f54u;
    constexpr RdramOffset kDmaDestination = 0x00004000u;

    machine.stage_cpu_gpr(kPiBaseIndex, kSyntheticPiMmioCpuBase);
    machine.stage_cpu_gpr(kMiBaseIndex, kSyntheticMiMmioCpuBase);
    trigger_pi_dma(
        machine,
        kPiDmaInstructionBase,
        kDmaDestination,
        kPiBaseIndex,
        kValueIndex,
        "pi_status_ack_demo_dma");
    require_rdram_word_equals(machine, kDmaDestination, 0xd1d4d7dau, "pi_status_ack_demo_copy");
    require_mi_register_equals(
        machine,
        kReadBase,
        kReadIndex,
    ...
    write_mi_register_through_cpu(
        ...
        kMiInterruptMaskRegisterOffset,
        kMiPendingPi,
        "pi_status_ack_demo_write_pi_mask");
    require_cop0_register_equals(
        machine,
        kCauseReadAddress,
        kCauseReadIndex,
        kCop0CauseRegisterIndex,
        kCop0CauseIp2,
        "pi_status_ack_demo_cause_ip2_before_ack");
    write_pi_register_through_cpu(
        machine,
        kSwPiStatusAddress,
        kValueIndex,
        kPiBaseIndex,
        kPiStatusRegisterOffset,
        kPiStatusClearInterrupt,
        "pi_status_ack_demo_ack");
    require_mi_register_equals(
        machine,
        kReadBase + 4u,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        0,
        "pi_status_ack_demo_pi_pending_cleared");
    require_cop0_register_equals(
        machine,
        kCauseReadAddress,
        kCauseReadIndex,
        kCop0CauseRegisterIndex,
        0,
        "pi_status_ack_demo_cause_ip2_after_ack");
    require_mi_register_equals(
        machine,
        kReadBase + 8u,
        kReadIndex,
        ...
        kMiInterruptMaskRegisterOffset,
        kMiPendingPi,
        "pi_status_ack_demo_mask_unchanged");
    require_pi_register_equals(
        machine,
        kReadBase + 12u,
        kReadIndex,
        ...
        kPiStatusRegisterOffset,
        0,
        "pi_status_ack_demo_status_idle");
    require_pi_register_equals(
        machine,
        kReadBase + 16u,
        kReadIndex,
        ...
        kPiDramRegisterOffset,
        kDmaDestination,
        "pi_status_ack_demo_dram_register_unchanged");
    require_pi_register_equals(
        machine,
        kReadBase + 20u,
        kReadIndex,
        ...
        kPiCartRegisterOffset,
        pi_cart_rom_address(0x00000040u),
        "pi_status_ack_demo_cart_register_unchanged");
    require_pi_register_equals(
        machine,
        kReadBase + 24u,
        kReadIndex,
        ...
        kPiCartToRdramLengthRegisterOffset,
        3u,
        "pi_status_ack_demo_length_register_unchanged");
  }

  {
    ...
    constexpr std::size_t kSpMemorySourceIndex = 10;
    constexpr std::size_t kSpMemoryReadIndex = 11;
    constexpr RdramOffset kSpReadInstructionBase = 0x00003f60u;
    constexpr RdramOffset kSwSpWriteLengthAddress = 0x00003f6cu;
    constexpr RdramOffset kSwSpStatusAddress = 0x00003f70u;
    constexpr RdramOffset kReadBase = 0x00003f74u;
    constexpr RdramOffset kSwMiMaskAddress = 0x00003f94u;
    constexpr RdramOffset kCauseReadAddress = 0x00003f98u;
    constexpr RdramOffset kLoadSpAddress = 0x00003f9cu;
    constexpr RdramOffset kStoreSpAddress = 0x00003fa0u;
    constexpr RdramOffset kSpReadSource = 0x00004080u;
    constexpr RdramOffset kSpWriteDestination = 0x000040a0u;
    constexpr std::uint32_t kSpDmemOffset = 0x00000200u;
    constexpr std::uint32_t kSpWriteDmemOffset = 0x00000240u;

    stage_sp_lw_instruction(machine, kLoadSpAddress, kSpMemoryReadIndex, kSpMemoryBaseIndex, 0);
    stage_sp_sw_instruction(machine, kStoreSpAddress, kSpMemorySourceIndex, kSpMemoryBaseIndex, 0);
    machine.stage_cpu_gpr(kSpBaseIndex, kSyntheticSpMmioCpuBase);
    machine.stage_cpu_gpr(kMiBaseIndex, kSyntheticMiMmioCpuBase);
    trigger_sp_read_dma(
        machine,
        kSpReadInstructionBase,
        kSpReadSource,
        kSpDmemOffset,
        kSpBaseIndex,
        kValueIndex,
        "sp_status_ack_demo_read_dma");
    machine.stage_cpu_gpr(kSpMemoryBaseIndex, sp_dmem_uncached_alias(kSpDmemOffset));
    step_at(machine, kLoadSpAddress, "sp_status_ack_demo_load_read_copy");
    require_gpr_equals(machine, kSpMemoryReadIndex, 0x11223344u, "sp_status_ack_demo_read_copy");
    require_mi_register_equals(
        machine,
        kReadBase,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingSp,
        "sp_status_ack_demo_sp_read_pending");
    ...
    write_mi_register_through_cpu(
        ...
        kMiInterruptMaskRegisterOffset,
        kMiPendingSp,
        "sp_status_ack_demo_write_sp_mask");
    require_cop0_register_equals(
        machine,
        kCauseReadAddress,
        kCauseReadIndex,
        kCop0CauseRegisterIndex,
        kCop0CauseIp2,
        "sp_status_ack_demo_cause_ip2_before_ack");
    write_sp_register_through_cpu(
        machine,
        kSwSpStatusAddress,
        kValueIndex,
        kSpBaseIndex,
        kSpStatusRegisterOffset,
        kSpStatusClearInterrupt,
        "sp_status_ack_demo_ack_read_pending");
    require_mi_register_equals(
        machine,
        kReadBase + 4u,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        0,
        "sp_status_ack_demo_sp_pending_cleared");
    require_cop0_register_equals(
        machine,
        kCauseReadAddress,
        kCauseReadIndex,
        kCop0CauseRegisterIndex,
        0,
        "sp_status_ack_demo_cause_ip2_after_ack");
    require_mi_register_equals(
        machine,
        kReadBase + 8u,
        kReadIndex,
        ...
        kMiInterruptMaskRegisterOffset,
        kMiPendingSp,
        "sp_status_ack_demo_mask_unchanged");
    require_sp_register_equals(
        machine,
        kReadBase + 12u,
        kReadIndex,
        ...
        kSpStatusRegisterOffset,
        0,
        "sp_status_ack_demo_status_idle");
    require_sp_register_equals(
        machine,
        kReadBase + 16u,
        kReadIndex,
        ...
        kSpMemoryRegisterOffset,
        kSpDmemOffset,
        "sp_status_ack_demo_mem_register_unchanged");
    require_sp_register_equals(
        machine,
        kReadBase + 20u,
        kReadIndex,
        ...
        kSpDramRegisterOffset,
        kSpReadSource,
        "sp_status_ack_demo_dram_register_unchanged");
    require_sp_register_equals(
        machine,
        kReadBase + 24u,
        kReadIndex,
        ...
        kSpReadLengthRegisterOffset,
        3u,
        "sp_status_ack_demo_read_length_unchanged");

    machine.stage_cpu_gpr(kSpMemoryBaseIndex, sp_dmem_uncached_alias(kSpWriteDmemOffset));
    machine.stage_cpu_gpr(kSpMemorySourceIndex, 0xaabbccddu);
    step_at(machine, kStoreSpAddress, "sp_status_ack_demo_seed_write_source");
    write_sp_register_through_cpu(
        machine,
        kSpReadInstructionBase,
        kValueIndex,
        ...
        kSpMemoryRegisterOffset,
        kSpWriteDmemOffset,
        "sp_status_ack_demo_sw_write_mem");
    write_sp_register_through_cpu(
        machine,
        kSpReadInstructionBase + 4u,
        kValueIndex,
        kSpBaseIndex,
        kSpDramRegisterOffset,
        kSpWriteDestination,
        "sp_status_ack_demo_sw_write_dram");
    write_sp_register_through_cpu(
        machine,
        kSwSpWriteLengthAddress,
        kValueIndex,
        ...
        kSpWriteLengthRegisterOffset,
        3u,
        "sp_status_ack_demo_write_dma");
    require_rdram_word_equals(machine, kSpWriteDestination, 0xaabbccddu, "sp_status_ack_demo_write_copy");
    require_mi_register_equals(
        machine,
        kReadBase,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingSp,
        "sp_status_ack_demo_sp_write_pending");
    write_sp_register_through_cpu(
        machine,
        kSwSpStatusAddress,
        kValueIndex,
        ...
        kSpStatusRegisterOffset,
        kSpStatusClearInterrupt,
        "sp_status_ack_demo_ack_write_pending");
    require_mi_register_equals(
        machine,
        kReadBase + 4u,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        0,
        "sp_status_ack_demo_sp_write_pending_cleared");
    require_rdram_word_equals(
        machine,
        kSpWriteDestination,
        0xaabbccddu,
        "sp_status_ack_demo_write_copy_preserved");
  }

  {
    ...
    constexpr std::size_t kValueIndex = 7;
    constexpr std::size_t kReadIndex = 8;
    constexpr RdramOffset kPiDmaInstructionBase = 0x000040c0u;
    constexpr RdramOffset kSpReadInstructionBase = 0x000040ccu;
    constexpr RdramOffset kSwPiStatusAddress = 0x000040d8u;
    constexpr RdramOffset kSwSpStatusAddress = 0x000040dcu;
    constexpr RdramOffset kSwMiMaskAddress = 0x000040e0u;
    constexpr RdramOffset kSwMiPendingAddress = 0x000040e4u;
    constexpr RdramOffset kLwMiPendingAddress = 0x000040e8u;
    constexpr RdramOffset kLwMiMaskAddress = 0x000040ecu;
    constexpr RdramOffset kDmaDestination = 0x00004180u;
    constexpr RdramOffset kSpReadSource = 0x000041a0u;
    ...
    write_mi_register_through_cpu(
        ...
        kMiInterruptMaskRegisterOffset,
        kMiSupportedBits,
        "pi_sp_status_ack_preserve_demo_write_all_mask");
    trigger_pi_dma(
        machine,
        kPiDmaInstructionBase,
        kDmaDestination,
        kPiBaseIndex,
        kValueIndex,
        "pi_sp_status_ack_preserve_demo_pi_dma");
    trigger_sp_read_dma(
        machine,
        kSpReadInstructionBase,
        kSpReadSource,
        ...
        kSpBaseIndex,
        kValueIndex,
        "pi_sp_status_ack_preserve_demo_sp_dma");
    require_mi_register_equals(
        machine,
        kLwMiPendingAddress,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingPi | kMiPendingSp,
        "pi_sp_status_ack_preserve_demo_both_pending");
    ...
    write_pi_register_through_cpu(
        ...
        kPiStatusRegisterOffset,
        kPiStatusClearInterrupt,
        "pi_sp_status_ack_preserve_demo_clear_pi");
    require_mi_register_equals(
        machine,
        kLwMiPendingAddress,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingSp,
        "pi_sp_status_ack_preserve_demo_sp_preserved");
    require_mi_register_equals(
        machine,
        kLwMiMaskAddress,
        kReadIndex,
        ...
        kMiInterruptMaskRegisterOffset,
        kMiSupportedBits,
        "pi_sp_status_ack_preserve_demo_mask_unchanged_after_pi_clear");

    trigger_pi_dma(
        machine,
        kPiDmaInstructionBase,
        kDmaDestination + 0x20u,
        kPiBaseIndex,
        kValueIndex,
        "pi_sp_status_ack_noop_demo_pi_dma_again");
    write_pi_register_through_cpu(
        machine,
        kSwPiStatusAddress,
        kValueIndex,
    ...
    write_pi_register_through_cpu(
        ...
        kPiBaseIndex,
        kPiStatusRegisterOffset,
        kPiStatusUnsupportedBits,
        "pi_status_ack_noop_demo_unsupported");
    require_mi_register_equals(
        machine,
        kLwMiPendingAddress,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingPi | kMiPendingSp,
        "pi_status_ack_noop_demo_pending_preserved");
    ...
    write_sp_register_through_cpu(
        ...
        kSpStatusRegisterOffset,
        kSpStatusClearInterrupt,
        "sp_status_ack_preserve_demo_clear_sp");
    require_mi_register_equals(
        machine,
        kLwMiPendingAddress,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingPi,
        "sp_status_ack_preserve_demo_pi_preserved");
    ...
    trigger_sp_read_dma(
        ...
        kSpBaseIndex,
        kValueIndex,
        "sp_status_ack_noop_demo_sp_dma_again");
    write_sp_register_through_cpu(
        machine,
        kSwSpStatusAddress,
        kValueIndex,
    ...
    write_sp_register_through_cpu(
        ...
        kSpBaseIndex,
        kSpStatusRegisterOffset,
        kSpStatusUnsupportedBits,
        "sp_status_ack_noop_demo_unsupported");
    write_sp_register_through_cpu(
        machine,
        kSwSpStatusAddress,
        kValueIndex,
        kSpBaseIndex,
        kSpStatusRegisterOffset,
        kSpStatusSetInterruptUnsupported,
        "sp_status_ack_noop_demo_set_interrupt_unsupported");
    require_mi_register_equals(
        machine,
        kLwMiPendingAddress,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingPi | kMiPendingSp,
        "sp_status_ack_noop_demo_pending_preserved");
    ...
    write_mi_register_through_cpu(
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingPi,
        "pi_sp_status_ack_mi_clear_demo_clear_pi");
    require_mi_register_equals(
        machine,
        kLwMiPendingAddress,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingSp,
        "pi_sp_status_ack_mi_clear_demo_clear_pi_only");
    write_mi_register_through_cpu(
        machine,
        kSwMiPendingAddress,
        kValueIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        kMiPendingSp,
        "pi_sp_status_ack_mi_clear_demo_clear_sp");
    require_mi_register_equals(
        machine,
        kLwMiPendingAddress,
        kReadIndex,
        ...
        kMiInterruptPendingRegisterOffset,
        0,
        "pi_sp_status_ack_mi_clear_demo_all_clear");
  }

  {
    ...
    constexpr std::size_t kValueIndex = 6;
    constexpr std::size_t kReadIndex = 7;
    constexpr RdramOffset kSwSpStatusAddress = 0x000041c0u;
    constexpr RdramOffset kLwMiPendingAddress = 0x000041c4u;
    ...
    write_sp_register_through_cpu(
        ...
        kSpStatusRegisterOffset,
        kSpStatusSetInterruptUnsupported,
        "sp_status_ack_set_interrupt_no_pending_demo");
    require_mi_register_equals(
        machine,
        kLwMiPendingAddress,
        kReadIndex,
  ...
  const auto require_device_interrupt_resume =
      [&](const char* label, bool use_pi_pending) {
        auto machine_storage = make_pi_dma_proof_machine();
        Machine& machine = *machine_storage;
        constexpr std::size_t kPiBaseIndex = 4;
        constexpr std::size_t kSpBaseIndex = 5;
        ...
        constexpr std::size_t kReadIndex = 9;
        constexpr RdramOffset kDeviceDmaInstructionBase = 0x00004200u;
        constexpr RdramOffset kSwMiMaskAddress = 0x00004210u;
        constexpr RdramOffset kStatusWriteAddress = 0x00004214u;
        constexpr RdramOffset kLwMiPendingAddress = 0x00004218u;
        constexpr RdramOffset kInterruptedAddress = 0x0000421cu;
        constexpr RdramOffset kVectorAckAddress = 0x00000180u;
        constexpr RdramOffset kVectorEretAddress = 0x00000184u;
        constexpr RdramOffset kDmaDataAddress = 0x00004280u;
        ...
        if (use_pi_pending) {
          trigger_pi_dma(
              machine,
              kDeviceDmaInstructionBase,
              kDmaDataAddress,
              kPiBaseIndex,
              kValueIndex,
              label);
          stage_pi_sw_instruction(machine, kVectorAckAddress, kValueIndex, kPiBaseIndex, kPiStatusRegisterOffset);
        } else {
          trigger_sp_read_dma(
              machine,
              kDeviceDmaInstructionBase,
              ...
              kValueIndex,
              label);
          stage_sp_sw_instruction(machine, kVectorAckAddress, kValueIndex, kSpBaseIndex, kSpStatusRegisterOffset);
        }

        machine.stage_rdram_u32_be(kVectorEretAddress, encode_cop0_eret());
        machine.stage_rdram_u32_be(kInterruptedAddress, encode_ori(kInterruptedIndex, 0, 0x6d6du));
        write_mi_register_through_cpu(
            machine,
            kSwMiMaskAddress,
            kValueIndex,
            kMiBaseIndex,
            kMiInterruptMaskRegisterOffset,
            use_pi_pending ? kMiPendingPi : kMiPendingSp,
            label);
        write_cop0_register_through_cpu(
            machine,
            kStatusWriteAddress,
            ...
            kCop0StatusIe | kCop0StatusInterruptMask2,
            label);
        machine.stage_cpu_gpr(
            kValueIndex,
            use_pi_pending ? kPiStatusClearInterrupt : kSpStatusClearInterrupt);
        ...
        require_stepped(machine.step_cpu_instruction(), label);
        require_stepped(machine.step_cpu_instruction(), label);
        if (machine.cpu_pc() != cpu_rdram_alias(kInterruptedAddress) ||
            machine.cpu_next_pc() != cpu_rdram_alias(kInterruptedAddress + 4u)) {
          throw std::runtime_error(std::string(label) + " ERET did not return to interrupted instruction");
        }
        require_stepped(machine.step_cpu_instruction(), label);
        require_gpr_equals(machine, kInterruptedIndex, 0x6d6du, label);
        require_mi_register_equals(
            machine,
            kLwMiPendingAddress,
            kReadIndex,
  ...
  require_device_interrupt_resume("pi_status_ack_interrupt_resume_demo", true);
  require_device_interrupt_resume("sp_status_ack_interrupt_resume_demo", false);
}
...
void run_cop0_address_error_gate_demo() {
...

  {
    ...
    constexpr CpuAddress kFaultingPc = 0x80003541u;
    ...
    require_step_machine_fault(
        machine,
        "cop0_address_error_gate_nonordinary_fetch_fault",
        MachineFaultKind::kUnalignedInstructionFetch,
        4);
    if (machine.cpu_pc() != kFaultingPc ||
        machine.cpu_next_pc() != kNonOrdinaryNextPc) {
      throw std::runtime_error("cop0_address_error_gate_nonordinary_fetch changed pc/next_pc");
    }
    require_cop0_register_equals(
        machine,
        0x000035a0u,
        ...
        kCop0BadVaddrRegisterIndex,
        0,
        "cop0_address_error_gate_nonordinary_fetch_badvaddr");
    require_cop0_register_equals(
        machine,
        0x000035a4u,
        kReadIndex,
        kCop0CauseRegisterIndex,
        0,
        "cop0_address_error_gate_nonordinary_fetch_cause");
  }
}
...
void run_cop0_control_transfer_alignment_gate_demo() {
...

  {
    ...
    Machine& machine = *machine_storage;
    constexpr std::size_t kReadIndex = 4;
    constexpr CpuAddress kFaultingPc = 0x80003b81u;
    ...
    require_step_machine_fault(
        machine,
        "cop0_control_align_gate_nonordinary_fetch_fault",
        MachineFaultKind::kUnalignedInstructionFetch,
        4);
    if (machine.cpu_pc() != kFaultingPc ||
        machine.cpu_next_pc() != kNonOrdinaryNextPc) {
      throw std::runtime_error("cop0_control_align_gate_nonordinary_fetch changed pc/next_pc");
    }
    require_cop0_register_equals(
        machine,
        0x00003ba0u,
        ...
        kCop0BadVaddrRegisterIndex,
        0,
        "cop0_control_align_gate_nonordinary_fetch_badvaddr");
    require_cop0_register_equals(
        machine,
        0x00003ba4u,
        kReadIndex,
        kCop0CauseRegisterIndex,
        0,
        "cop0_control_align_gate_nonordinary_fetch_cause");
  }
}
...
void run_cop0_delay_slot_exception_entry_demo() {
  std::cout << "fn64 bootstrap COP0 demo: delay-slot faults record local BD state\n";

  require_cop0_register_equals(
      *std::make_unique<Machine>(Cartridge{}),
      0x00003c80u,
      4,
      kCop0CauseRegisterIndex,
      0,
      "cop0_delay_slot_initial_cause");

  {
    ...
    constexpr std::size_t kCauseReadIndex = 8;
    constexpr std::size_t kStatusReadIndex = 9;
    constexpr std::size_t kBadVaddrReadIndex = 10;
    constexpr std::size_t kCountReadIndex = 11;
    constexpr std::size_t kTargetMarkerIndex = 12;
    constexpr RdramOffset kCountWriteAddress = 0x00003ca0u;
    constexpr RdramOffset kBranchAddress = 0x00003cc0u;
    constexpr RdramOffset kDelayAddress = kBranchAddress + 4u;
    constexpr RdramOffset kBranchTargetAddress = 0x00003d20u;
    constexpr RdramOffset kVectorInstructionAddress = 0x00000180u;
    constexpr CpuRegisterValue kResultSentinel = 0x9988776655443322ull;
    ...
    write_cop0_register_through_cpu(
        ...
        kCop0CountRegisterIndex,
        300,
        "cop0_delay_slot_overflow_count");
    machine.stage_rdram_u32_be(kBranchAddress, encode_j(cpu_rdram_alias(kBranchTargetAddress)));
    machine.stage_rdram_u32_be(
        kDelayAddress,
        encode_addi(
    ...
    machine.stage_rdram_u32_be(
        kBranchTargetAddress,
        encode_ori(static_cast<std::uint8_t>(kTargetMarkerIndex), 0, 0x6d6du));
    machine.stage_rdram_u32_be(
        kVectorInstructionAddress,
        encode_mfc0(static_cast<std::uint8_t>(kEpcReadIndex), kCop0EpcRegisterIndex));
    machine.stage_rdram_u32_be(
    ...
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_overflow_branch");
    if (machine.cpu_pc() != cpu_rdram_alias(kDelayAddress) ||
        machine.cpu_next_pc() != cpu_rdram_alias(kBranchTargetAddress)) {
      throw std::runtime_error("cop0_delay_slot_overflow did not enter delay-slot cadence");
    }
    require_exception(machine.step_cpu_instruction(), "cop0_delay_slot_overflow_entry");
    require_gpr_equals(
        machine,
        kResultIndex,
        kResultSentinel,
        "cop0_delay_slot_overflow_no_commit");
    require_gpr_equals(machine, kTargetMarkerIndex, 0, "cop0_delay_slot_overflow_target_not_run");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_overflow_vector_epc");
    require_gpr_equals(
        machine,
        kEpcReadIndex,
        cpu_value_from_sign_extended_u32(cpu_rdram_alias(kBranchAddress)),
        "cop0_delay_slot_overflow_epc_is_branch");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_overflow_vector_cause");
    require_gpr_equals(
        machine,
        kCauseReadIndex,
        cpu_value_from_sign_extended_u32(
            kCop0CauseBranchDelay | kCop0CauseExcCodeOverflowBits),
        "cop0_delay_slot_overflow_cause_bd");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_overflow_vector_status");
    require_gpr_equals(machine, kStatusReadIndex, kCop0StatusExl, "cop0_delay_slot_overflow_status");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_overflow_vector_badvaddr");
    require_gpr_equals(machine, kBadVaddrReadIndex, 0, "cop0_delay_slot_overflow_badvaddr");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_overflow_vector_count");
    require_gpr_equals(machine, kCountReadIndex, 306, "cop0_delay_slot_overflow_count_no_tick");
    require_stopped(machine.step_cpu_instruction(), "cop0_delay_slot_overflow_vector_break");
  }

  {
    ...
    constexpr std::size_t kBadVaddrReadIndex = 7;
    constexpr std::size_t kCauseReadIndex = 8;
    constexpr RdramOffset kBranchAddress = 0x00003d80u;
    constexpr RdramOffset kDelayAddress = kBranchAddress + 4u;
    constexpr RdramOffset kBranchTargetAddress = 0x00003dc0u;
    constexpr RdramOffset kDataAddress = 0x00003de0u;
    constexpr RdramOffset kVectorInstructionAddress = 0x00000180u;
    constexpr CpuAddress kBadAddress = cpu_rdram_alias(kDataAddress) + 1u;
    constexpr CpuRegisterValue kTargetSentinel = 0x0102030405060708ull;
    ...
    machine.stage_rdram_u32_be(
        kBranchTargetAddress,
        encode_ori(static_cast<std::uint8_t>(kTargetIndex), 0, 0x1111u));
    machine.stage_rdram_u32_be(kDataAddress, 0xaabbccddu);
    machine.stage_rdram_u32_be(
        kVectorInstructionAddress,
        encode_mfc0(static_cast<std::uint8_t>(kEpcReadIndex), kCop0EpcRegisterIndex));
    machine.stage_rdram_u32_be(
    ...
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_lw_branch");
    require_exception(machine.step_cpu_instruction(), "cop0_delay_slot_lw_entry");
    require_gpr_equals(machine, kTargetIndex, kTargetSentinel, "cop0_delay_slot_lw_no_commit");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_lw_vector_epc");
    require_gpr_equals(
        machine,
        kEpcReadIndex,
        cpu_value_from_sign_extended_u32(cpu_rdram_alias(kBranchAddress)),
        "cop0_delay_slot_lw_epc_is_branch");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_lw_vector_badvaddr");
    require_gpr_equals(
        machine,
        kBadVaddrReadIndex,
        cpu_value_from_sign_extended_u32(kBadAddress),
        "cop0_delay_slot_lw_badvaddr");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_lw_vector_cause");
    require_gpr_equals(
        machine,
        kCauseReadIndex,
        cpu_value_from_sign_extended_u32(kCop0CauseBranchDelay | kCop0CauseExcCodeAdelBits),
        "cop0_delay_slot_lw_cause_bd");
    require_stopped(machine.step_cpu_instruction(), "cop0_delay_slot_lw_vector_break");
  }

  {
    ...
    constexpr std::size_t kBadVaddrReadIndex = 6;
    constexpr std::size_t kCauseReadIndex = 7;
    constexpr RdramOffset kBranchAddress = 0x00003e20u;
    constexpr RdramOffset kDelayAddress = kBranchAddress + 4u;
    constexpr RdramOffset kBranchTargetAddress = 0x00003e60u;
    constexpr RdramOffset kDataAddress = 0x00003e80u;
    constexpr RdramOffset kVectorInstructionAddress = 0x00000180u;
    constexpr CpuAddress kBadAddress = cpu_rdram_alias(kDataAddress) + 1u;
    constexpr std::uint32_t kDataSentinel = 0x55667788u;
    ...
    machine.stage_rdram_u32_be(
        ...
        encode_sw(
            ...
            static_cast<std::uint8_t>(kBaseIndex),
            1));
    machine.stage_rdram_u32_be(kDataAddress, kDataSentinel);
    machine.stage_rdram_u32_be(
        kVectorInstructionAddress,
        encode_mfc0(static_cast<std::uint8_t>(kBadVaddrReadIndex), kCop0BadVaddrRegisterIndex));
    machine.stage_rdram_u32_be(
    ...
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_sw_branch");
    require_exception(machine.step_cpu_instruction(), "cop0_delay_slot_sw_entry");
    require_rdram_word_equals(machine, kDataAddress, kDataSentinel, "cop0_delay_slot_sw_no_store");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_sw_vector_badvaddr");
    require_gpr_equals(
        machine,
        kBadVaddrReadIndex,
        cpu_value_from_sign_extended_u32(kBadAddress),
        "cop0_delay_slot_sw_badvaddr");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_sw_vector_cause");
    require_gpr_equals(
        machine,
        kCauseReadIndex,
        cpu_value_from_sign_extended_u32(kCop0CauseBranchDelay | kCop0CauseExcCodeAdesBits),
        "cop0_delay_slot_sw_cause_bd");
    require_stopped(machine.step_cpu_instruction(), "cop0_delay_slot_sw_vector_break");
  }

  {
    ...
    constexpr std::size_t kBadVaddrReadIndex = 6;
    constexpr std::size_t kCauseReadIndex = 7;
    constexpr RdramOffset kBranchAddress = 0x00003ec0u;
    constexpr RdramOffset kDelayAddress = kBranchAddress + 4u;
    constexpr RdramOffset kBranchTargetAddress = 0x00003f00u;
    constexpr RdramOffset kVectorInstructionAddress = 0x00000180u;
    constexpr CpuAddress kBadTarget = 0x80003f42u;
    constexpr CpuRegisterValue kLinkSentinel = 0x8877665544332211ull;
    ...
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_jalr_branch");
    require_exception(machine.step_cpu_instruction(), "cop0_delay_slot_jalr_entry");
    require_gpr_equals(machine, kLinkIndex, kLinkSentinel, "cop0_delay_slot_jalr_link_no_commit");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_jalr_vector_badvaddr");
    require_gpr_equals(
        machine,
        kBadVaddrReadIndex,
        cpu_value_from_sign_extended_u32(kBadTarget),
        "cop0_delay_slot_jalr_badvaddr");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_jalr_vector_cause");
    require_gpr_equals(
        machine,
        kCauseReadIndex,
        cpu_value_from_sign_extended_u32(kCop0CauseBranchDelay | kCop0CauseExcCodeAdelBits),
        "cop0_delay_slot_jalr_cause_bd");
    require_stopped(machine.step_cpu_instruction(), "cop0_delay_slot_jalr_vector_break");
  }
}

void run_cop0_delay_slot_exception_handler_demo() {
  std::cout << "fn64 bootstrap COP0 demo: delay-slot handlers own EPC recovery policy\n";

  {
    ...
    constexpr std::size_t kBaseIndex = 4;
    constexpr std::size_t kFaultTargetIndex = 5;
    constexpr std::size_t kAfterPairIndex = 6;
    constexpr std::size_t kBranchTargetIndex = 7;
    constexpr std::size_t kEpcSourceIndex = 8;
    constexpr RdramOffset kBranchAddress = 0x00003f80u;
    constexpr RdramOffset kDelayAddress = kBranchAddress + 4u;
    constexpr RdramOffset kAfterPairAddress = kBranchAddress + 8u;
    constexpr RdramOffset kBranchTargetAddress = 0x00003fc0u;
    constexpr RdramOffset kDataAddress = 0x00003fe0u;
    constexpr RdramOffset kVectorInstructionAddress = 0x00000180u;
    constexpr CpuRegisterValue kFaultTargetSentinel = 0xa5a55a5adeadbeefull;
    ...
    machine.stage_rdram_u32_be(
        kAfterPairAddress,
        encode_ori(static_cast<std::uint8_t>(kAfterPairIndex), 0, 0x7171u));
    machine.stage_rdram_u32_be(
        kBranchTargetAddress,
        encode_ori(static_cast<std::uint8_t>(kBranchTargetIndex), 0, 0x7272u));
    machine.stage_rdram_u32_be(kDataAddress, 0x11223344u);
    machine.stage_rdram_u32_be(
        kVectorInstructionAddress,
        encode_mtc0(static_cast<std::uint8_t>(kEpcSourceIndex), kCop0EpcRegisterIndex));
    ...
    machine.stage_cpu_gpr(kBaseIndex, cpu_rdram_alias(kDataAddress));
    machine.stage_cpu_gpr(kFaultTargetIndex, kFaultTargetSentinel);
    machine.stage_cpu_gpr(kAfterPairIndex, 0);
    machine.stage_cpu_gpr(kBranchTargetIndex, 0);
    machine.stage_cpu_gpr(kEpcSourceIndex, cpu_rdram_alias(kAfterPairAddress));
    machine.stage_cpu_pc(cpu_rdram_alias(kBranchAddress));
    machine.stage_cpu_next_pc(cpu_rdram_alias(kDelayAddress));

    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_handler_skip_branch");
    require_exception(machine.step_cpu_instruction(), "cop0_delay_slot_handler_skip_entry");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_handler_skip_write_epc");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_handler_skip_eret");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_handler_skip_after_pair");
    require_gpr_equals(
        machine,
        kFaultTargetIndex,
        kFaultTargetSentinel,
        "cop0_delay_slot_handler_skip_fault_target");
    require_gpr_equals(machine, kAfterPairIndex, 0x7171u, "cop0_delay_slot_handler_skip_after");
    require_gpr_equals(
        machine,
        kBranchTargetIndex,
        0,
        "cop0_delay_slot_handler_skip_original_target_not_run");
    require_cop0_register_equals(
        machine,
        0x00003fa0u,
        kAfterPairIndex,
        kCop0StatusRegisterIndex,
        0,
        "cop0_delay_slot_handler_skip_status_exl_cleared");
  }

  {
    ...
    constexpr std::size_t kLoadTargetIndex = 5;
    constexpr std::size_t kBranchTargetIndex = 6;
    constexpr RdramOffset kBranchAddress = 0x00004020u;
    constexpr RdramOffset kDelayAddress = kBranchAddress + 4u;
    constexpr RdramOffset kBranchTargetAddress = 0x00004060u;
    constexpr RdramOffset kDataAddress = 0x00004080u;
    constexpr RdramOffset kVectorInstructionAddress = 0x00000180u;
    ...
    machine.stage_rdram_u32_be(
        kBranchTargetAddress,
        encode_ori(static_cast<std::uint8_t>(kBranchTargetIndex), 0, 0x7373u));
    machine.stage_rdram_u32_be(kDataAddress, 0x01020304u);
    machine.stage_rdram_u32_be(
        kVectorInstructionAddress,
        encode_lui(static_cast<std::uint8_t>(kBaseIndex), 0x8000u));
    ...
    machine.stage_rdram_u32_be(kVectorInstructionAddress + 8u, encode_cop0_eret());
    machine.stage_cpu_gpr(kBaseIndex, cpu_rdram_alias(kDataAddress) + 1u);
    machine.stage_cpu_gpr(kLoadTargetIndex, 0);
    machine.stage_cpu_gpr(kBranchTargetIndex, 0);
    machine.stage_cpu_pc(cpu_rdram_alias(kBranchAddress));
    machine.stage_cpu_next_pc(cpu_rdram_alias(kDelayAddress));

    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_handler_retry_branch");
    require_exception(machine.step_cpu_instruction(), "cop0_delay_slot_handler_retry_entry");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_handler_retry_fix_high");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_handler_retry_fix_low");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_handler_retry_eret");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_handler_retry_branch_again");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_handler_retry_delay");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_handler_retry_target");
    require_gpr_equals(
        machine,
        kLoadTargetIndex,
        cpu_value_from_sign_extended_u32(0x01020304u),
        "cop0_delay_slot_handler_retry_load");
    require_gpr_equals(machine, kBranchTargetIndex, 0x7373u, "cop0_delay_slot_handler_retry_target");
  }
}

void run_cop0_delay_slot_exception_boundary_demo() {
  std::cout << "fn64 bootstrap COP0 demo: delay-slot BD ownership stays narrow\n";

  {
    ...
    constexpr std::size_t kTargetIndex = 5;
    constexpr std::size_t kReadIndex = 6;
    constexpr RdramOffset kBranchAddress = 0x00004100u;
    constexpr RdramOffset kDelayAddress = kBranchAddress + 4u;
    constexpr RdramOffset kBranchTargetAddress = 0x00004140u;
    constexpr RdramOffset kDataAddress = 0x00004180u;
    constexpr RdramOffset kOrdinaryOverflowAddress = 0x000041c0u;
    ...
    machine.stage_rdram_u32_be(
        ...
        encode_lw(
            ...
            static_cast<std::uint8_t>(kBaseIndex),
            1));
    machine.stage_rdram_u32_be(kDataAddress, 0x12345678u);
    machine.stage_cpu_gpr(kBaseIndex, cpu_rdram_alias(kDataAddress));
    machine.stage_cpu_pc(cpu_rdram_alias(kBranchAddress));
    machine.stage_cpu_next_pc(cpu_rdram_alias(kDelayAddress));
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_ordinary_clear_seed_branch");
    require_exception(machine.step_cpu_instruction(), "cop0_delay_slot_ordinary_clear_seed");
    require_cop0_register_equals(
        machine,
        0x00004120u,
        kReadIndex,
        kCop0CauseRegisterIndex,
        kCop0CauseBranchDelay | kCop0CauseExcCodeAdelBits,
        "cop0_delay_slot_ordinary_clear_seed_cause");
    ...
    write_cop0_register_through_cpu(
        ...
        kCop0StatusRegisterIndex,
        0,
        "cop0_delay_slot_ordinary_clear_status");
    machine.stage_rdram_u32_be(
        kOrdinaryOverflowAddress,
        encode_addi(static_cast<std::uint8_t>(kTargetIndex), static_cast<std::uint8_t>(kBaseIndex), 1));
    machine.stage_cpu_gpr(kBaseIndex, 0x7fffffffu);
    machine.stage_cpu_pc(cpu_rdram_alias(kOrdinaryOverflowAddress));
    machine.stage_cpu_next_pc(cpu_rdram_alias(kOrdinaryOverflowAddress + 4u));
    require_exception(machine.step_cpu_instruction(), "cop0_delay_slot_ordinary_clear_overflow");
    require_cop0_register_equals(
        machine,
        0x00004128u,
        kReadIndex,
        kCop0CauseRegisterIndex,
        kCop0CauseExcCodeOverflowBits,
        "cop0_delay_slot_ordinary_exception_clears_bd");
  }

  {
    ...
    constexpr std::size_t kTargetIndex = 5;
    constexpr std::size_t kReadIndex = 6;
    constexpr RdramOffset kBranchAddress = 0x00004200u;
    constexpr RdramOffset kDelayAddress = kBranchAddress + 4u;
    constexpr RdramOffset kBranchTargetAddress = 0x00004240u;
    constexpr RdramOffset kDataAddress = 0x00004280u;
    constexpr RdramOffset kVectorInstructionAddress = 0x00000180u;
    ...
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_cause_clear_branch");
    require_exception(machine.step_cpu_instruction(), "cop0_delay_slot_cause_clear_entry");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_cause_clear_mtc0_zero");
    require_cop0_register_equals(
        machine,
        0x00004220u,
        kReadIndex,
        kCop0CauseRegisterIndex,
        kCop0CauseBranchDelay | kCop0CauseExcCodeAdelBits,
        "cop0_delay_slot_cause_clear_preserves_bd_exc_code");
  }

  {
    ...
    constexpr std::size_t kResultIndex = 6;
    constexpr std::size_t kReadIndex = 7;
    constexpr RdramOffset kStatusWriteAddress = 0x000042c0u;
    constexpr RdramOffset kBranchAddress = 0x000042e0u;
    constexpr RdramOffset kDelayAddress = kBranchAddress + 4u;
    constexpr RdramOffset kBranchTargetAddress = 0x00004320u;
    ...
    write_cop0_register_through_cpu(
        ...
        kCop0StatusRegisterIndex,
        kCop0StatusExl,
        "cop0_delay_slot_exl_gate_status");
    machine.stage_rdram_u32_be(kBranchAddress, encode_j(cpu_rdram_alias(kBranchTargetAddress)));
    machine.stage_rdram_u32_be(
        kDelayAddress,
        encode_addi(
    ...
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_exl_gate_branch");
    require_step_machine_fault(
        machine,
        "cop0_delay_slot_exl_gate_fault",
        MachineFaultKind::kSignedArithmeticOverflow,
        0);
    require_gpr_equals(machine, kResultIndex, kResultSentinel, "cop0_delay_slot_exl_gate_no_commit");
    require_cop0_register_equals(
        machine,
        0x00004300u,
        kReadIndex,
        kCop0CauseRegisterIndex,
        0,
        "cop0_delay_slot_exl_gate_cause_unchanged");
    require_cop0_register_equals(
        machine,
        0x00004304u,
        kReadIndex,
        kCop0StatusRegisterIndex,
        kCop0StatusExl,
        "cop0_delay_slot_exl_gate_status_preserved");
  }

  {
    ...
    constexpr std::size_t kStatusSourceIndex = 5;
    constexpr std::size_t kCauseReadIndex = 6;
    constexpr RdramOffset kBranchAddress = 0x00004340u;
    constexpr RdramOffset kDelayAddress = kBranchAddress + 4u;
    constexpr RdramOffset kBranchTargetAddress = 0x00004380u;
    constexpr RdramOffset kInterruptInstructionAddress = 0x000043a0u;
    constexpr RdramOffset kVectorInstructionAddress = 0x00000180u;
    ...
    machine.stage_rdram_u32_be(
        kDelayAddress,
        encode_addi(
            static_cast<std::uint8_t>(kCauseReadIndex),
            static_cast<std::uint8_t>(kCauseReadIndex),
            0x0001u));
    machine.stage_rdram_u32_be(kVectorInstructionAddress, encode_mfc0(static_cast<std::uint8_t>(kCauseReadIndex), kCop0CauseRegisterIndex));
    machine.stage_cpu_gpr(kCauseReadIndex, 0x7fffffffu);
    machine.stage_cpu_pc(cpu_rdram_alias(kBranchAddress));
    machine.stage_cpu_next_pc(cpu_rdram_alias(kDelayAddress));
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_interrupt_clear_seed_branch");
    require_exception(machine.step_cpu_instruction(), "cop0_delay_slot_interrupt_clear_seed");
    write_cop0_register_through_cpu(
        machine,
        0x00004350u,
        kStatusSourceIndex,
        kCop0StatusRegisterIndex,
        kCop0StatusIe | kCop0StatusInterruptMask0,
        "cop0_delay_slot_interrupt_clear_status");
    write_cop0_register_through_cpu(
        machine,
        0x00004354u,
        kCauseSourceIndex,
        kCop0CauseRegisterIndex,
        kCop0CauseIp0,
        "cop0_delay_slot_interrupt_clear_cause");
    machine.stage_rdram_u32_be(
        kInterruptInstructionAddress,
        encode_ori(static_cast<std::uint8_t>(kStatusSourceIndex), 0, 0x7a7au));
    machine.stage_cpu_pc(cpu_rdram_alias(kInterruptInstructionAddress));
    machine.stage_cpu_next_pc(cpu_rdram_alias(kInterruptInstructionAddress + 4u));
    require_interrupted(machine.step_cpu_instruction(), "cop0_delay_slot_interrupt_clears_bd");
    require_stepped(machine.step_cpu_instruction(), "cop0_delay_slot_interrupt_cause_read");
    require_gpr_equals(machine, kCauseReadIndex, kCop0CauseIp0, "cop0_delay_slot_interrupt_bd_clear");
  }

  {
    ...
    Machine& machine = *machine_storage;
    constexpr std::size_t kReadIndex = 4;
    constexpr CpuAddress kUnalignedPc = 0x800043e1u;
    constexpr CpuAddress kNonOrdinaryNextPc = 0x80004420u;

    machine.stage_cpu_pc(kUnalignedPc);
    machine.stage_cpu_next_pc(kNonOrdinaryNextPc);
    require_step_machine_fault(
        machine,
        "cop0_delay_slot_unaligned_fetch_guard",
        MachineFaultKind::kUnalignedInstructionFetch,
        4);
    if (machine.cpu_pc() != kUnalignedPc ||
        machine.cpu_next_pc() != kNonOrdinaryNextPc) {
      throw std::runtime_error("cop0_delay_slot_unaligned_fetch_guard changed pc/next_pc");
    }
    require_cop0_register_equals(
        machine,
        0x00004400u,
        ...
        kCop0CauseRegisterIndex,
        0,
        "cop0_delay_slot_unaligned_fetch_guard_cause");
  }
}
...
void run_cop0_signed_overflow_gate_demo() {
...

  {
    ...
    Machine& machine = *machine_storage;
    constexpr std::size_t kReadIndex = 4;
    constexpr CpuAddress kFaultingPc = 0x80002c81u;
    ...
    require_step_machine_fault(
        machine,
        "cop0_overflow_gate_nonordinary_fetch_fault",
        MachineFaultKind::kUnalignedInstructionFetch,
        4);
    if (machine.cpu_pc() != kFaultingPc ||
        machine.cpu_next_pc() != kNonOrdinaryNextPc) {
      throw std::runtime_error("cop0_overflow_gate_nonordinary_fetch changed pc/next_pc");
    }
    require_cop0_register_equals(
        machine,
        0x00002ca0u,
        ...
        kCop0EpcRegisterIndex,
        0,
        "cop0_overflow_gate_nonordinary_fetch_epc_unchanged");
    require_cop0_register_equals(
        machine,
        0x00002ca4u,
        kReadIndex,
        kCop0CauseRegisterIndex,
        0,
        "cop0_overflow_gate_nonordinary_fetch_cause_unchanged");
  }
}
...
void run_cop0_signed_overflow_fault_boundary_demo() {
...

  {
    ...
    constexpr RdramOffset kStatusWriteAddress = 0x00002eb8u;
    constexpr std::size_t kBaseIndex = 4;
    constexpr std::size_t kTargetIndex = 5;
    write_cop0_register_through_cpu(
        machine,
        ...
        kCop0StatusRegisterIndex,
        kCop0StatusExl,
        "cop0_overflow_boundary_unaligned_data_exl");
    machine.stage_rdram_u32_be(
        kLwAddress,
        encode_lw(
            static_cast<std::uint8_t>(kTargetIndex),
...

  {
    ...
    constexpr RdramOffset kStatusWriteAddress = 0x00002ef8u;
    constexpr std::size_t kTargetIndex = 4;
    write_cop0_register_through_cpu(
        machine,
        kStatusWriteAddress,
        ...
        kCop0StatusRegisterIndex,
        kCop0StatusExl,
        "cop0_overflow_boundary_control_exl");
...
void run_load_link_store_conditional_fault_demo() {
  ...
  const auto require_fault_case =
      [](const char* label,
        ...
        constexpr RdramOffset kStatusWriteAddress = 0x00000de0u;
...

  {
    ...
    constexpr std::size_t kLlTargetIndex = 5;
    constexpr std::size_t kScSourceIndex = 7;
    constexpr RdramOffset kStatusWriteAddress = 0x00000e3cu;
    ...
    write_cop0_register_through_cpu(
        ...
        kCop0StatusRegisterIndex,
        kCop0StatusExl,
        "ll_fault_demo_sc_preserve_reservation_status");
    machine.stage_cpu_gpr(kBaseIndex, cpu_rdram_alias(kDataAddress));
    machine.stage_cpu_gpr(kScSourceIndex, 0xaabbccddu);
    machine.stage_cpu_pc(cpu_rdram_alias(kBadScAddress));
...
void run_data_demos(Machine& machine) {
  ...
  run_cop0_delay_slot_exception_entry_demo();
  run_cop0_delay_slot_exception_handler_demo();
  run_cop0_delay_slot_exception_boundary_demo();
