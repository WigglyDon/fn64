...
pub const RSP_SCALAR_SW_OPCODE: u8 = 0x2b;
pub const RSP_SCALAR_SW_BYTE_COUNT: usize = 4;
pub const RSP_VECTOR_LOAD_OPCODE: u8 = 0x32;
pub const RSP_VECTOR_LQV_SUBOPCODE: u8 = 4;
pub const RSP_VECTOR_STORE_OPCODE: u8 = 0x3a;
pub const RSP_VECTOR_SQV_SUBOPCODE: u8 = 4;
...
impl MachineRspLastInstructionState {
    ...
    pub const fn destination_gpr(self) -> Option<u8> {
        match self.destination {
            ...
            | MachineRspLastInstructionDestination::VectorSqv { .. }
            ...
            | MachineRspLastInstructionDestination::ScalarSw { .. }
    ...
    pub const fn source_gpr(self) -> Option<u8> {
        match self.destination {
            MachineRspLastInstructionDestination::ScalarMtc0 { source_gpr, .. }
            | MachineRspLastInstructionDestination::ScalarSw { source_gpr, .. } => Some(source_gpr),
            _ => None,
        }
    }
    ...
    pub const fn source_vector(self) -> Option<u8> {
        match self.destination {
            MachineRspLastInstructionDestination::VectorSqv { source_vector, .. } => {
                Some(source_vector)
            }
            _ => None,
        }
...
impl MachineRspStepOutcome {
    pub const fn identity(self) -> MachineRspInstructionIdentity {
        match self {
            ...
            Self::VectorSqvCommitted { .. } => MachineRspInstructionIdentity::Sqv,
            ...
            Self::ScalarSwCommitted { .. } => MachineRspInstructionIdentity::Sw,
    ...
    pub const fn instruction_pc(self) -> u16 {
        match self {
            ...
            | Self::VectorSqvCommitted { instruction_pc, .. }
            ...
            | Self::ScalarSwCommitted { instruction_pc, .. }
...
pub enum MachineRspStepRejectionReason {
    ...
    ScalarSwBaseUnavailable {
        base_gpr: u8,
    },
    ScalarSwSourceUnavailable {
        source_gpr: u8,
    },
    ScalarSwAddressMisaligned {
        local_dmem_address: u16,
    },
    ScalarSwDmemRangeMalformed {
        local_dmem_address: u16,
    },
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.reason {
            ...
            MachineRspStepRejectionReason::SqvDmemRangeMalformed {
                ...
                f,
                "RSP Sqv DMEM range is malformed at local address 0x{local_dmem_address:03x} for {byte_count} bytes"
            ),
            ...
            MachineRspStepRejectionReason::VectorStoreUnsupported {
                ...
                f,
                "RSP vector-store sub-operation {subopcode} with element {element} is not represented"
            ),
            ...
            MachineRspStepRejectionReason::ScalarSwBaseUnavailable { base_gpr } => {
                write!(f, "RSP scalar Sw base r{base_gpr} is unavailable")
            }
            MachineRspStepRejectionReason::ScalarSwSourceUnavailable { source_gpr } => {
                write!(f, "RSP scalar Sw source r{source_gpr} is unavailable")
            }
            MachineRspStepRejectionReason::ScalarSwAddressMisaligned {
                local_dmem_address,
            } => write!(
                f,
                "RSP scalar Sw local DMEM address 0x{local_dmem_address:03x} is outside the aligned word boundary"
            ),
            MachineRspStepRejectionReason::ScalarSwDmemRangeMalformed {
                local_dmem_address,
            } => write!(
                f,
                "RSP scalar Sw DMEM range is malformed at local address 0x{local_dmem_address:03x}"
            ),
...
pub(crate) struct MachineRspSqvPlan {
    ...
    base_source: MachineRspScalarRegisterSource,
    source_vector: u8,
    source_state: MachineRspVectorRegisterState,
    element: u8,
    signed_offset: i8,
    local_dmem_address: u16,
...
impl MachineRspSqvPlan {
    ...
    pub(crate) fn dmem_provenance(&self) -> MachineSpDmemRspSqvProvenance {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct MachineRspScalarSwPlan {
    instruction_pc: u16,
    old_next_pc: u16,
    base_gpr: u8,
    base_value: u32,
    ...
    signed_offset: i16,
    local_dmem_address: u16,
    byte_provenance: [SpImemByteProvenance; RSP_SCALAR_SW_BYTE_COUNT],
}

impl MachineRspScalarSwPlan {
    pub(crate) const fn instruction_pc(&self) -> u16 {
        self.instruction_pc
    }
    ...
    pub(crate) const fn stored_word(&self) -> u32 {
        self.source_value
    }

    pub(crate) fn dmem_provenance(&self) -> MachineSpDmemRspStoreWordProvenance {
        MachineSpDmemRspStoreWordProvenance::new(
            self.instruction_pc,
            classify_instruction_source(self.byte_provenance),
            (self.base_gpr, self.base_value),
            (self.source_gpr, self.source_value),
            self.signed_offset,
            self.local_dmem_address,
        )
    }
...
impl MachineRspExecutionState {
    ...
    pub(crate) fn decode(
        ...
        if opcode == RSP_VECTOR_STORE_OPCODE {
            let subopcode = ((raw_word >> 11) & 0x1f) as u8;
            let element = ((raw_word >> 7) & 0x0f) as u8;
            if subopcode == RSP_VECTOR_SQV_SUBOPCODE {
                let raw_offset = (raw_word & 0x7f) as u8;
                let signed_offset = ((raw_offset << 1) as i8) >> 1;
                return Ok(MachineRspDecodedInstruction::Sqv {
                    base_gpr: ((raw_word >> 21) & 0x1f) as u8,
                    source_vector: ((raw_word >> 16) & 0x1f) as u8,
                    element,
                    signed_offset,
                });
            }
        ...
        if opcode == RSP_SCALAR_SW_OPCODE {
            return Ok(MachineRspDecodedInstruction::Sw {
                base_gpr: ((raw_word >> 21) & 0x1f) as u8,
                source_gpr: ((raw_word >> 16) & 0x1f) as u8,
                signed_offset: raw_word as u16 as i16,
            });
    ...
    pub(crate) fn plan_sqv(
        &self,
        instruction_pc: u16,
        decoded: MachineRspDecodedInstruction,
        byte_provenance: [SpImemByteProvenance; 4],
    ) -> Result<MachineRspSqvPlan, MachineRspStepRejection> {
        let MachineRspDecodedInstruction::Sqv {
            base_gpr,
            source_vector,
            element,
            signed_offset,
            ...
            unreachable!("Sqv planner receives only decoded Sqv")
        };
        let MachineRspScalarRegisterState::Available {
            value: base_value,
            source: base_source,
    ...
    pub(crate) fn apply_sqv(&mut self, plan: MachineRspSqvPlan) -> MachineRspStepOutcome {
        self.next_pc = Some(sequential_local_pc(plan.old_next_pc));
        self.delay_slot_context = None;
        self.committed_instruction_count = self.committed_instruction_count.wrapping_add(1);
        self.last_instruction = Some(MachineRspLastInstructionState {
            instruction_pc: plan.instruction_pc,
            identity: MachineRspInstructionIdentity::Sqv,
            destination: MachineRspLastInstructionDestination::VectorSqv {
                source_vector: plan.source_vector,
                local_dmem_address: plan.local_dmem_address,
            },
            byte_provenance: plan.byte_provenance,
        });
        MachineRspStepOutcome::VectorSqvCommitted {
            instruction_pc: plan.instruction_pc,
            source_vector: plan.source_vector,
            local_dmem_address: plan.local_dmem_address,
    ...
    pub(crate) fn plan_sw(
        &self,
        instruction_pc: u16,
        decoded: MachineRspDecodedInstruction,
        byte_provenance: [SpImemByteProvenance; RSP_SCALAR_SW_BYTE_COUNT],
    ) -> Result<MachineRspScalarSwPlan, MachineRspStepRejection> {
        let MachineRspDecodedInstruction::Sw {
            base_gpr,
            source_gpr,
            signed_offset,
        } = decoded
        else {
            unreachable!("scalar Sw planner receives only decoded Sw")
        };
        let MachineRspScalarRegisterState::Available {
            value: base_value,
            source: base_source,
        ...
        if local_dmem_address & RSP_INSTRUCTION_ALIGNMENT_MASK != 0 {
            return Err(MachineRspStepRejection::new(
                MachineRspStepRejectionReason::ScalarSwAddressMisaligned { local_dmem_address },
            ));
        }
        Ok(MachineRspScalarSwPlan {
            instruction_pc,
            old_next_pc: self
                .next_pc
                .unwrap_or_else(|| sequential_local_pc(instruction_pc)),
    ...
    pub(crate) fn apply_sw(&mut self, plan: MachineRspScalarSwPlan) -> MachineRspStepOutcome {
        self.next_pc = Some(sequential_local_pc(plan.old_next_pc));
        self.delay_slot_context = None;
        self.committed_instruction_count = self.committed_instruction_count.wrapping_add(1);
        self.last_instruction = Some(MachineRspLastInstructionState {
            instruction_pc: plan.instruction_pc,
            identity: MachineRspInstructionIdentity::Sw,
            destination: MachineRspLastInstructionDestination::ScalarSw {
                source_gpr: plan.source_gpr,
                local_dmem_address: plan.local_dmem_address,
            },
            byte_provenance: plan.byte_provenance,
        });
        MachineRspStepOutcome::ScalarSwCommitted {
            instruction_pc: plan.instruction_pc,
            source_gpr: plan.source_gpr,
            local_dmem_address: plan.local_dmem_address,
            stored_word: plan.source_value,
        }
    }
...
mod tests {
    ...
    const fn sqv_word(base_gpr: u8, source_vector: u8, element: u8, signed_offset: i8) -> u32 {
        ((RSP_VECTOR_STORE_OPCODE as u32) << 26)
            | ((base_gpr as u32) << 21)
            | ((source_vector as u32) << 16)
            | ((RSP_VECTOR_SQV_SUBOPCODE as u32) << 11)
            | ((element as u32) << 7)
            | ((signed_offset as u8 & 0x7f) as u32)
    }
    ...
    const fn sw_word(base_gpr: u8, source_gpr: u8, signed_offset: i16) -> u32 {
        ((RSP_SCALAR_SW_OPCODE as u32) << 26)
            | ((base_gpr as u32) << 21)
    ...
    #[test]
    fn rsp_sqv_element_zero_plans_public_quad_boundary_and_commits_only_rsp_cadence() {
        let mut rsp = MachineRspExecutionState::default();
        rsp.synchronize_pc_write(0x020);
        stage_available_scalar(&mut rsp, 3, 0xabcd_f027);
        let bytes = core::array::from_fn(|index| 0x91_u8.wrapping_add(index as u8 * 7));
        stage_available_vector(&mut rsp, 12, bytes);
        assert_eq!(
            rsp.decode(sqv_word(3, 12, 0, 0)),
            Ok(MachineRspDecodedInstruction::Sqv {
                base_gpr: 3,
                source_vector: 12,
                element: 0,
                signed_offset: 0,
        ...
        let plan = rsp
            .plan_sqv(
                0x020,
                rsp.decode(sqv_word(3, 12, 0, 0)).unwrap(),
                TEST_INSTRUCTION_PROVENANCE,
            )
            .unwrap();
        assert_eq!(plan.base_gpr, 3);
        assert_eq!(plan.base_value, 0xabcd_f027);
        assert!(matches!(
            plan.base_source,
            MachineRspScalarRegisterSource::Mfc0(_)
        ));
        assert_eq!(plan.source_vector, 12);
        assert_eq!(plan.source_state, vector_before.registers[12]);
        ...
        assert_eq!(rsp.vector_unit, vector_before);
        assert_eq!(rsp.accumulator_and_flags, accumulator_before);
        assert_eq!(rsp.next_pc(), Some(0x028));
        assert_eq!(rsp.delay_slot_context(), None);
        assert_eq!(rsp.committed_instruction_count(), 1);
        let last = rsp.last_instruction().unwrap();
        assert_eq!(last.identity(), MachineRspInstructionIdentity::Sqv);
        assert_eq!(last.source_vector(), Some(12));
        assert_eq!(last.local_dmem_address(), Some(0x027));
        assert_eq!(last.destination_vector(), None);
    }

    #[test]
    fn rsp_sqv_scaled_wrapping_address_and_atomic_source_boundaries_are_exact() {
        let mut rsp = MachineRspExecutionState::default();
        rsp.synchronize_pc_write(0);
        ...
        let negative = rsp
            .plan_sqv(
                0,
                rsp.decode(sqv_word(0, 12, 0, -1)).unwrap(),
                TEST_INSTRUCTION_PROVENANCE,
            )
            .unwrap();
        assert_eq!(negative.local_dmem_address(), 0xff0);
        assert_eq!(negative.byte_count(), 16);

        stage_available_scalar(&mut rsp, 3, 0xffff_0fff);
        let final_byte = rsp
            .plan_sqv(
                0,
                rsp.decode(sqv_word(3, 12, 0, 0)).unwrap(),
            ...
            .unwrap();
        assert_eq!(final_byte.local_dmem_address(), 0xfff);
        assert_eq!(final_byte.byte_count(), 1);

        stage_available_scalar(&mut rsp, 4, 0xffff_0ff0);
        let wrapped = rsp
            .plan_sqv(
                0,
                rsp.decode(sqv_word(4, 12, 0, 1)).unwrap(),
                TEST_INSTRUCTION_PROVENANCE,
            )
            .unwrap();
        ...
        for (word, expected) in [
            ...
            (
                sqv_word(0, 12, 1, 0),
                MachineRspStepRejectionReason::SqvElementUnsupported { element: 1 },
            ),
        ] {
            let before = rsp.clone();
            assert_eq!(
                rsp.plan_sqv(0, rsp.decode(word).unwrap(), TEST_INSTRUCTION_PROVENANCE)
                    .unwrap_err()
                    .reason(),
                expected
            );
    ...
    #[test]
    fn rsp_sw_address_sources_successor_and_atomic_rejections_are_exact() {
        let mut rsp = MachineRspExecutionState::default();
        rsp.synchronize_pc_write(0x020);
        stage_available_scalar(&mut rsp, 3, 0xabcd_f040);
        stage_available_scalar(&mut rsp, 4, 0x1122_3344);
        assert_eq!(
            rsp.decode(sw_word(3, 4, 0)),
            Ok(MachineRspDecodedInstruction::Sw {
                base_gpr: 3,
                source_gpr: 4,
                signed_offset: 0,
            })
        ...
        let plan = rsp
            .plan_sw(
                0x020,
                rsp.decode(sw_word(3, 4, 0)).unwrap(),
                TEST_INSTRUCTION_PROVENANCE,
            )
            .unwrap();
        assert_eq!(plan.base_gpr, 3);
        assert_eq!(plan.base_value, 0xabcd_f040);
        assert!(matches!(
            plan.base_source,
            MachineRspScalarRegisterSource::Mfc0(_)
        ));
        assert_eq!(plan.source_gpr, 4);
        assert_eq!(plan.source_value, 0x1122_3344);
        assert!(matches!(
            plan.source,
            MachineRspScalarRegisterSource::Mfc0(_)
        ));
        assert_eq!(plan.signed_offset, 0);
        assert_eq!(plan.local_dmem_address(), 0x040);
        assert_eq!(plan.stored_word(), 0x1122_3344);
        assert_eq!(
            rsp.apply_sw(plan),
            MachineRspStepOutcome::ScalarSwCommitted {
                instruction_pc: 0x020,
        ...
        assert_eq!(
            rsp.last_instruction().unwrap().identity(),
            MachineRspInstructionIdentity::Sw
        );
        assert_eq!(rsp.last_instruction().unwrap().source_gpr(), Some(4));
        assert_eq!(
            rsp.last_instruction().unwrap().local_dmem_address(),
            Some(0x040)
        );

        let mut alias = MachineRspExecutionState::default();
        alias.synchronize_pc_write(0x030);
        stage_available_scalar(&mut alias, 5, 0xffff_0ffc);
        let plan = alias
            .plan_sw(
                0x030,
                alias.decode(sw_word(5, 5, 4)).unwrap(),
                TEST_INSTRUCTION_PROVENANCE,
            )
            .unwrap();
        assert_eq!(plan.local_dmem_address(), 0);
        assert_eq!(plan.stored_word(), 0xffff_0ffc);

        let zero = MachineRspExecutionState::default();
        let plan = zero
            .plan_sw(
                0,
                zero.decode(sw_word(0, 0, 0x40)).unwrap(),
                TEST_INSTRUCTION_PROVENANCE,
            )
            .unwrap();
        assert_eq!(plan.local_dmem_address(), 0x040);
        assert_eq!(plan.stored_word(), 0);

        for (word, expected) in [
            (
                sw_word(1, 0, 0),
                MachineRspStepRejectionReason::ScalarSwBaseUnavailable { base_gpr: 1 },
            ),
            (
                sw_word(0, 1, 0),
                MachineRspStepRejectionReason::ScalarSwSourceUnavailable { source_gpr: 1 },
            ),
            (
                sw_word(0, 0, 2),
                MachineRspStepRejectionReason::ScalarSwAddressMisaligned {
            ...
            let candidate = MachineRspExecutionState::default();
            let before = candidate.clone();
            assert_eq!(
                candidate
                    .plan_sw(
                        0,
                        candidate.decode(word).unwrap(),
                        TEST_INSTRUCTION_PROVENANCE
                    )
                    .unwrap_err()
                    .reason(),
    ...
    fn rsp_vector_load_store_consumer_and_other_scalar_memory_frontiers_remain_closed() {
        ...
        assert_eq!(
            rsp.decode(0xa004_0040).unwrap_err().reason(),
            MachineRspStepRejectionReason::ScalarStoreUnsupported { opcode: 0x28 }
        );
    }
}
