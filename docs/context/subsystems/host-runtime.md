...
both identities are fixed redacted placeholders in diagnostics. Without
`--pif-rom`, the host selects typed `CleanRoomHle` and Machine stages the
post-boot cartridge-entry transition. With `--pif-rom`, it selects the preserved
typed `ExplicitPifFirmware` low-level path. The public-synthetic profile is
unreachable from this shell. The probe prints a bounded redacted dashboard,
enforces a positive step ceiling, and uses structural value-free failure
classifications.
