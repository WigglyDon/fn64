...
- the reset model names a non-boot CPU reset vector and explicitly treats its PIF ROM fetch target as unavailable, not cartridge execution or N64 reset/PIF boot
- CPU instruction fetch currently uses KSEG0/KSEG1-style direct RDRAM aliases plus a narrow local SP DMEM direct-alias seam
...
## Non-boot reset vector

A newly constructed machine now deliberately resets into a local non-boot state:
- powered_on is true
- RDRAM is zeroed
- CPU pc is 0xbfc00000
- CPU next_pc is 0xbfc00004
- CPU hi/lo are zero
- CPU general registers are zero

This reset vector is not cartridge execution, IPL3 execution, PIF boot ROM emulation, or CIC/security behavior. Because no PIF ROM bytes are modeled, the reset fetch from 0xbfc00000 is recognized as a local unavailable PIF ROM fetch seam whose direct physical target is 0x1fc00000. Stepping from reset reports through the existing local fetch-address exception path instead of executing boot, cartridge, RDRAM, or SP DMEM bytes.
...
CPU data load/store can access Machine-owned 4 KiB SP DMEM and 4 KiB SP IMEM byte memories through direct aliases. CPU instruction fetch can now read aligned 32-bit instruction words from the local SP DMEM direct-alias range, including 0xa4000040. SP IMEM remains data-addressable only and is not instruction-fetchable.
